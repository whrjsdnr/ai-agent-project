...
class ResearchMeasurementStatus(StrEnum):
    MEASURED = "measured"
...
class ResearchExecutionMetadata(BaseModel):
    model_config = ConfigDict(frozen=True)
...
class ResearchMetricObservation(BaseModel):
    model_config = ConfigDict(frozen=True)

    metric_id: str = Field(min_length=1)
    value: float | int | str | None = None
    unit: str | None = None
    status: ResearchMeasurementStatus
    notes: str | None = None

    _validate_metric_id = field_validator("metric_id")(_nonblank_identifier)

    @model_validator(mode="after")
    def validate_measurement(self) -> "ResearchMetricObservation":
        if (
            self.status is ResearchMeasurementStatus.NOT_MEASURED
            and self.value is not None
        ):
            raise ValueError("Not-measured metrics must not contain a measured value")
        if self.status is ResearchMeasurementStatus.MEASURED and self.value is None:
            raise ValueError("Measured metrics require a supplied value")
        return self


class ResearchBaselineObservation(BaseModel):
    model_config = ConfigDict(frozen=True)

    name: str = Field(min_length=1)
    metrics: tuple[ResearchMetricObservation, ...] = ()
...
class ResearchTaskResult(BaseModel):
    ...
    methodology_step_ids: tuple[str, ...] = ()
    metric_ids: tuple[str, ...] = ()
    execution_status: ResearchTaskExecutionStatus
...
class ResearchResultSubmission(BaseModel):
    """Immutable user-supplied empirical data; never generated by an LLM."""
    ...
    execution_metadata: ResearchExecutionMetadata = Field(
        default_factory=ResearchExecutionMetadata
    )
    task_results: tuple[ResearchTaskResult, ...] = ()
    metric_observations: tuple[ResearchMetricObservation, ...] = ()
    baseline_observations: tuple[ResearchBaselineObservation, ...] = ()
    user_observations: tuple[str, ...] = ()
    missing_items: tuple[str, ...] = ()
    raw_source_name: str | None = None

    _validate_run_id = field_validator("research_run_id")(_nonblank_identifier)

    @model_validator(mode="after")
    def validate_unique_observations(self) -> "ResearchResultSubmission":
        task_ids = {result.task_id for result in self.task_results}
        metric_ids = {item.metric_id for item in self.metric_observations}
        if len(task_ids) != len(self.task_results):
            raise ValueError("Duplicate research task results are not allowed")
        if len(metric_ids) != len(self.metric_observations):
            raise ValueError("Duplicate research metric observations are not allowed")
        return self


class ResearchAssessmentStatus(StrEnum):
    MET = "met"
    NOT_MET = "not_met"
...
class ResearchObjectiveAssessmentStatus(StrEnum):
    SUPPORTED = "supported"
...
class ResearchMetricAssessmentPayload(BaseModel):
    model_config = ConfigDict(frozen=True)
    metric_id: str = Field(min_length=1)
    assessment: ResearchAssessmentStatus
    rationale: str
...
class ResearchSuccessCriterionAssessment(BaseModel):
    model_config = ConfigDict(frozen=True)
    ...
    _validate_id = field_validator("criterion_id")(_nonblank_identifier)


class ResearchObjectiveAssessment(BaseModel):
    model_config = ConfigDict(frozen=True)
    objective_id: str = Field(min_length=1)
    assessment: ResearchObjectiveAssessmentStatus
    rationale: str
    evidence_refs: tuple[str, ...] = ()
...
class ResearchFinding(BaseModel):
    ...
    _validate_id = field_validator("finding_id")(_nonblank_identifier)


class ResearchResultAnalysisPayload(BaseModel):
    """Interpretation-only strict LLM output; observations remain authoritative."""

    model_config = ConfigDict(frozen=True)
    metric_assessments: tuple[ResearchMetricAssessmentPayload, ...] = ()
    success_criterion_assessments: tuple[ResearchSuccessCriterionAssessment, ...] = ()
    objective_assessments: tuple[ResearchObjectiveAssessment, ...] = ()
...
class ResearchMetricAssessment(ResearchMetricAssessmentPayload):
    observed_value: float | int | str | None = None
    observation_status: ResearchMeasurementStatus


class ResearchResultAnalysis(BaseModel):
    model_config = ConfigDict(frozen=True)
    objective_assessments: tuple[ResearchObjectiveAssessment, ...] = ()
    metric_assessments: tuple[ResearchMetricAssessment, ...] = ()
    success_criterion_assessments: tuple[ResearchSuccessCriterionAssessment, ...] = ()
    findings: tuple[ResearchFinding, ...] = ()
    anomalies: tuple[str, ...] = ()
    limitations: tuple[str, ...] = ()
...
class ResearchRun(BaseModel):
    ...
    def validate_selection(self) -> "ResearchRun":
        ...
        if self.result_submission is not None:
            if self.implementation_plan is None or self.plan_revision_state is None:
                raise ValueError("Research results require implementation state")
            if (
                self.result_submission.approved_plan_version
                != self.plan_revision_state.active_version
            ):
                raise ValueError("Research results changed approved plan version")
        if self.result_analysis is not None and self.result_submission is None:
            raise ValueError("Research result analysis requires submitted results")
        if self.status is ResearchStatus.RESEARCH_RESULTS_SUBMITTED and (
            self.result_submission is None
        ):
            raise ValueError("Submitted-results status requires a submission")
        if self.status is ResearchStatus.RESEARCH_RESULTS_ANALYZED and (
